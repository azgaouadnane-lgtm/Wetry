...
<head>
    <meta charset="UTF-8">
    <title>Dashboard | SpiderMod Admin</title>
    ...
    <div class="container">
        <div class="card">
            <div class="card-header">
                <h2>Manage Apps</h2>
                <a href="add.php" class="btn btn-primary">+ Add New App</a>
            </div>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                    <?php if (empty($apps)): ?>
                    <tr><td colspan="6" style="text-align:center;">No apps found.</td></tr>
                    <?php else: ?>
                        <?php foreach (array_reverse($apps) as $app): ?>
                        <tr>
                            <td><?= $app['id'] ?></td>
                            <td><img src="<?= sanitize($app['icon_app']) ?>" class="app-icon-thumb" alt=""></td>
                            <td><strong><?= sanitize($app['name']) ?></strong></td>
                            <td><?= sanitize($app['category']) ?></td>
                            <td><?= count($app['geo'] ?? []) ?></td>
                            <td class="actions">
                                <form action="delete.php" method="post" class="actions-group" onsubmit="return confirm('Delete this app?');">
                                    <a href="edit.php?id=<?= $app['id'] ?>" class="btn btn-sm btn-primary">Edit</a>
                                    <?= csrfField() ?>
